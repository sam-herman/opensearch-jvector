...
public interface KNNScoringSpace {
    ...
    class InnerProd extends KNNFieldSpace {
        ...
        public BiFunction<?, ?, Float> getScoringMethod(final Object processedQuery) {
            if (processedQuery instanceof float[]) {
                return (float[] q, float[] v) -> KNNScoringSpaceUtil.normalizeScore(-KNNScoringUtil.innerProduct(q, v));
            } else {
                return (byte[] q, byte[] v) -> KNNScoringSpaceUtil.normalizeScore(-KNNScoringUtil.innerProduct(q, v));
            }
        }
    }
